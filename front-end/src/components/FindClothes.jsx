...
function FindClothes() {
    ...
    const renderResultsStage = () => {
        ...
        const getProxiedImageUrl = (originalUrl) => {
            ...
            // Use specialized endpoint for Amazon images
            if (fixedUrl.includes('amazon.com') || fixedUrl.includes('media-amazon.com')) {
                return `http://localhost:5000/api/amazon-image?url=${encodeURIComponent(fixedUrl)}`;
            }
            
            // For other external images
            return `http://localhost:5000/api/image-proxy?url=${encodeURIComponent(fixedUrl)}`;
        };
                        ...
                        >
                            <img 
                                ...
                                onError={(e) => {
                                    ...
                                    // Try the local placeholder first
                                    e.target.src = "/assets/placeholder-product.jpg";
                                    // Fallback to an external service if the local file fails
                                    e.target.onerror = () => {
                                      e.target.src = `https://placehold.co/400x500/gray/white?text=${encodeURIComponent(product.type || 'Product')}`;
